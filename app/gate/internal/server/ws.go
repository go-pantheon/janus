...
func NewWebSocketServer(c *conf.Server, logger log.Logger, rt *router.RouteTable, svc *service.Service) (*ws.Server, error) {
	if rt == nil {
	...
	var opts = []server.Option{
		server.WithReadFilter(
			middleware.Chain(
				...
				tracing.Server(),
				metrics.Server(),
				logging.Request(xnet.NetKindWebSocket),
			),
		),
		server.WithWriteFilter(
			middleware.Chain(
				logging.Reply(xnet.NetKindWebSocket),
			),
		),
	}
	...
	s, err := ws.NewServer(c.Ws.Addr, c.Ws.Path, svc, opts...)
	if err != nil {
		return nil, errors.Wrap(err, "create WebSocket server failed")
	}
